...
public class ClassEx08_연습1 {
	...
	int player = 0;
	for (int i = 0; i < object.game.length; i++) {
		if (object.game[i] == 2) {
			player = i;
		}
	}
	...
	while (true) {
		...
		for (int i = 0; i < object.game.length; i++) {
			System.out.print(object.game[i] + " ");
		}
		System.out.println();
		...
		if (move == 1) {
			...
			if (player != 0 && object.game[player - 1] == 1) {
			...
			object.game[player] = 0;
			player--;
			
			if (player  == -1) {
				player = object.game.length - 1;
			}
			
			object.game[player] = 2;
		}
		else if (move == 2) {
			if (player != object.game.length-1 && object.game[player + 1] == 1) {
